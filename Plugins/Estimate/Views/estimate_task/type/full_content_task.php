...
<?php
echo view(
        "Estimate\Views\\estimate_task\\form_add_new_estimate_task",
        array(
            'task_id' => $task_id,
            'estimate_id_prefix' => $estimate_id_prefix,
            'user_id' => $user_id
        )
);
?>
...
<div class="table" id="task-estimate-data-table-container">
    <table id="task-estimate-data-table" class="display" cellspacing="0" width="100%">
    </table>
</div>
<script type="text/javascript">
    ...
    $(document).ready(function (){
        $("#task-estimate-data-table").appTable({
            source: '<?php echo_uri("estimate_task/list_data_table") ?>/' + <?php echo $task_id ?>,
            serverSide: true,
