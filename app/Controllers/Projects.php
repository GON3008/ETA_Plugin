...
class Projects extends Security_Controller
{
    ...
    public function list_data()
    {
        ...
        foreach ($list_data as $data) {
            $row_data = $this->_make_row($data, $custom_fields);
            $result[] = $row_data;
        }

        $eta_data = $this->list_data_eta();
        $result = array_merge($result, $eta_data);

        echo json_encode(array("data" => $result));
    }

    public function list_data_eta(){
       $eta_list = app_hooks()->do_action('app_hook_eta_list_data_extension');
    }
    ...
    function view($project_id = 0, $tab = "")
    {
        ...
        $view_data["show_document_info"] = (get_setting("module_document")) ? true : false;
    ...
    function documents($project_id)
    {
        validate_numeric_value($project_id);
        $this->access_only_team_members();
        $view_data['project_id'] = $project_id;
        return $this->template->view("projects/documents/index", $view_data);
    }
